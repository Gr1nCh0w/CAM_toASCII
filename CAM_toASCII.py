...
def main():
  ...
  while rval:
    ...
    key = cv2.waitKey(50) # 50ms pause -> ~20fps mais c'est suffisant, fait pas le fou
    if key == 27:
      break

def toASCII(frame, cols = 180, rows = 60):
...
def grayToChar(gray):
  CHAR_LIST = ' .:-=+*#%@'    # Choisis les caractères que tu veux, t'as le choix.
